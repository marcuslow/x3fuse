...
- **DP1X, DP2X and SD15 DNG thumbnails come from the camera JPEG.** Finder, Quick Look, Photos and Lightroom show a DNG's embedded preview at thumbnail sizes. For these three bodies the preview is the camera's own embedded JPEG, downscaled to 1600 px and stored as a JPEG-compressed DNG preview, the layout Adobe's DNG Converter uses (fix.10, narrowed to these models in fix.13). Other cameras keep the converter's rendered preview. The raw data is unchanged. In the fork's [x3fuse-core](https://github.com/marcuslow/x3fuse-core).
