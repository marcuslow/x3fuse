...
- **DP1X, DP2X and SD15 no longer render green.** These 2010-11 bodies' stored white-balance gains fall about 7 percent short of red and their colour matrices amplify that into a heavy green cast in every DNG reader and in the TIFF/JPEG output. Since fix.12 the converter applies a per-model gain correction fitted against Adobe DNG Converter's rendering of the same files (Auto white balance; Flash-preset shots under tungsten light are still off). Fork-only, in [x3fuse-core](https://github.com/marcuslow/x3fuse-core) (`e286256`).
- **DP1X, DP2X and SD15 DNG thumbnails come from the camera JPEG.** Finder, Quick Look, Photos and Lightroom show a DNG's embedded preview at thumbnail sizes. For these three bodies the preview is the camera's own embedded JPEG, downscaled to 1600 px and stored as a JPEG-compressed DNG preview, the layout Adobe's DNG Converter uses. Other cameras keep the converter's rendered preview, now at least 640 px wide (upstream caps it at 300). The raw data is unchanged. In the fork's [x3fuse-core](https://github.com/marcuslow/x3fuse-core).
