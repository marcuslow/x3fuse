...
- **TRUE II white balance** (fork x3fuse-core since `e286256`, fork-only because the numbers are
  fitted to Adobe's output): `x3f_get_gain` multiplies the CAMF gains by a per-model factor for
